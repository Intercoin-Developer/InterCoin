    ...
    {   
        ...
        *phash = block.GetHashYespower();
        ...
        if ((nNonce & 0xfff) == 0 || miner->fKill > 0) {
            if (miner->fKill > 0)
                LogPrintf("InterCoin miner kill flag > 0\n");
            return false;
        }
    }
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash()) {
            LogPrintf ("InterCoin Miner: generated block is stale\n");
            return false;
        }
    }
    ...
    if (!ProcessNewBlock (chainparams, ptr, true, nullptr)) {
        LogPrintf ("InterCoin Miner: ProcessNewBlock, block not accepted\n");
        result =  false;
    }
...
void static ITCMiner(MinerInfo* miner, const CChainParams& chainparams)
{
    LogPrintf("InterCoin Miner started\n");
    SetThreadPriority (THREAD_PRIORITY_LOWEST);
    RenameThread ("intercoin-miner");
    ...
    try {
        while (true) {
            ...
            {
                LogPrintf("InterCoin Miner runtime error: %s\n", e.what());
                LogPrintf("InterCoin Miner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                MilliSleep(4*1000);
                vpwallets[0]->GetScriptForMining (coinbaseScript);
                continue;
            }
            ...
            LogPrintf("Running InterCoin Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true) {
                ...
                if (ScanHash (miner, pblock, nNonce, &hash))
                {
                    if (UintToArith256(hash) <= hashTarget)
                    {
                        ...
                        LogPrintf("InterCoin Miner: proof-of-work found  \n  hash: %s  \ntarget: %s\n",
    ...
    {
        LogPrintf("InterCoin Miner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
...
void GenerateITC(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ITCMiner, vMiners[i], boost::cref(chainparams)));
